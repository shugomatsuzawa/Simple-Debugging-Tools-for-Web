...
- Titleタグ・URLの取得
    - 現在開いているタブのTitleタグと、エンコードされたURLを取得し、コピーしやすいフィールドを表示します。
- 外部サイト移動アラート
    - あなたが指定したドメインと異なるページに移動した際に画面上部でお知らせします。  
      例えばステージング環境でWebサイトのテストをする際、誤って本番環境に接続されているリンクをタップした場合、この機能を使えばすぐに気づくことができます。
- クイックアドミン
    - 現在開いているページのURLをもとに、あらかじめ登録したパスやポートを自動で付加し、新しいタブで開く機能です。
      /wp-admin や :8000 などをワンクリックで開けるため、管理画面や開発用URLへの移動を素早く行えます。
...
# ビルドする
npm install
npx @tailwindcss/cli -i ./tailwind.css -o ./style.css --watch
```
